...
class kyc_user extends Controller
{
    ...
    public function kyc_upload_id(Request $request){
        $validatedData = $request->validate([
            'id_proof' => 'required|image|mimes:jpeg,png,jpg,gif', 
            'address_proof' => 'required|image|mimes:jpeg,png,jpg,gif', 
        ]);
        $currentDate = Carbon::now();
        $user = Auth::user();
        $uploadedImage_front = $request->file('id_proof');
        $uploadedImage_back = $request->file('address_proof');
        ...
        $kyc->save();
        return redirect()->route('kyc_upload_page')->with('success', 'ID successfully uploaded. Hold on for approval');
